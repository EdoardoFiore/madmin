...
from .protocol import (
    ...
    ACTION_BACKUP_RUN, ACTION_BACKUP_RESTORE, ACTION_BACKUP_LIST,
    ACTION_FIREWALL_RELOAD, ACTION_INFO,
    make_result,
)
...
async def dispatch_command(ws, payload: dict):
    ...
    try:
        ...
        elif action == ACTION_BACKUP_LIST:
            success, result, error = await _handle_backup_list(params)
...
async def _handle_backup_list(params: dict):
    from modules.agent.service.ops import run_backup_list
    ok, result = await run_backup_list(params)
    return ok, result, None if ok else result.get("error")
