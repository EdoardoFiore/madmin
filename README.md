...
| `-l`, `--provision-lan [ifaces]` | Auto-provisions a managed LAN for special setups where the LAN must stay **editable only externally** (e.g. its IP assigned via cloud-init). Accepts an optional comma-separated interface list (e.g. `--provision-lan eth1,eth2`): **all listed interfaces are locked read-only** in the Network section, and the **first** one is the managed LAN that gets DHCP + LAN→WAN NAT (MASQUERADE). Without a value, the managed interface is auto-detected by name (`eth1`/`ens19`) and only it is locked. MADMIN does **not** set the IP — it is assigned externally; the bound DHCP derives its subnet/gateway from the interface's live IP. If a required interface is not present, provisioning is skipped (warning printed at the end of setup), as if the flag had not been passed. Self-heals on every boot. |
